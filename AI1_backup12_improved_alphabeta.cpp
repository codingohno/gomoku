...
std::array<int,572> my_stra_win;
std::array<int,572> enemy_stra_win;
...
///endgame func
//evaluate current board status
bool endgame(list_of_movement given_list){
    //usually when we call endgame the board is already modified
    //however we use win array to make it faster
    ...
    //we have my win
    //make a copy
    std::array<int,572> my_temp=my_stra_win;

    //we have enemy win
    //make a copy
    std::array<int,572> enemy_temp=enemy_stra_win;

    std::set<int> modified_set;
        ...
        if(single_movement[2]!=0){
            for(int way=0;way<572;way++){
                if(winning_methods[single_movement[0]/*row*/][single_movement[1]/*col*/][way]){
                    ...
                    modified_set.insert(way);
                    //std::cout<<" "<<modified_set.size()<<std::endl;
    ...
    //check game over
    for(auto modify_way:modified_set){
        //might be a bug
        if((my_temp[modify_way]==5)||(enemy_temp[modify_way]==5)){
            return true;
        }
    }
...
svf_result state_value_function(list_of_movement given_list){
    ...
    for(auto score:enemy_temp){
        if(score>0){
            //different values given to each winning
            if(score==2){player_mark-=4;}
            else if(score==3){player_mark-=400;}
            else if(score==4){player_mark-=400000;}
            else if(score==5){player_mark-=40000000;wrap_result.gameover=-1;}
        }
    }
...
void write_valid_spot(std::ofstream& fout) {
    ...
    while(true/*&&init_depth<=3*/){
        //call minimax with depth
        movement initial_dummy_move={-2,-2,0};
        ...
        //std::cout<<to_choose_result.chosen_movement[0]<<" "<<to_choose_result.chosen_movement[1]<<" alphabetaanswer\n";

        if(true/*best_value<to_choose_result.minimax_value comparison isnt need because alpha beta gives up the best choice*/){
            bool test_in_range=(to_choose_result.chosen_movement[0]>=0)&&(to_choose_result.chosen_movement[1]>=0)&&(to_choose_result.chosen_movement[0]<SIZE)&&(to_choose_result.chosen_movement[1]<SIZE);

                if(test_in_range&&(board[to_choose_result.chosen_movement[0]][to_choose_result.chosen_movement[1]]==EMPTY)){
        ...
        ///print
        //if the input is valid move //only can print
        ...
        ///not legal then print other thing else
        else{
            //std::cout<<row<<" "<<col<<" illegal\n";
            //random
...
minimax_result alphabeta(list_of_movement given_list, int depth,long long int alpha,long long int beta, bool maximizingPlayer){
    ...
    //append and check for gameover
    //append
    auto single_movement =given_list.back();
    if(single_movement[2]!=0){board[single_movement[0]][single_movement[1]]=single_movement[2];}
    ...
    //check
    bool leaf=endgame(given_list);

    //base case
    if((depth ==0)||leaf){//depth==0 or gameover
        //evaluate
        svf_result svf_evaluation_result=state_value_function(given_list);
        int evaluation_result=svf_evaluation_result.svf_value;
        value.minimax_value=evaluation_result;
    ...
    //recursive cases
        ...
        if (maximizingPlayer){
            //init the ret type
            value.minimax_value=std::numeric_limits<int>::max()*(-1);
            //put in current step name into the result
            ///cannot be returneddddd!!!!!!//must be changed
            value.chosen_movement=given_list.back();
            ...
            for(auto unit:next_move_set){
                ...
                //function call
                minimax_result temp_result=alphabeta(carry_on, depth - 1,alpha,beta, false);


                //compare value and get greater one
                if((value.minimax_value)<(temp_result.minimax_value)){
                    if(MM_DEBUG){
                        std::string log="original maximizing value max:";
                        log+=std::to_string(value.minimax_value);
                    ...
                    value.minimax_value=temp_result.minimax_value;
                    //put in child node name
                    value.chosen_movement=unit;// row col player
            ...
            ///after exploring restore the board
            ///restore value
            if(single_movement[2]!=0){board[single_movement[0]][single_movement[1]]=EMPTY;}
            ...
            for(auto unit:next_move_set){
                ...
                //function call
                minimax_result temp_result=alphabeta(carry_on, depth - 1, alpha,beta,true);

                //compare values
                if((value.minimax_value)>(temp_result.minimax_value)){
                    value.minimax_value=temp_result.minimax_value;
                    value.chosen_movement=unit;//row col opponent
                }
