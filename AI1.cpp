...
long long int state_value_function(state check_state){
    //count our own
    //4 data
    std::array<std::array<int, 2>,SIZE> row;row.fill({});
...
void write_valid_spot(std::ofstream& fout,int row,int col) {
    srand(time(NULL));
...
int main(int, char** argv) {
    ...
    //get my best move from minimax
    if(occu_location.size()==0){
        write_valid_spot(fout,7,7);
    }
    else{
        //reset log
        movement initial_dummy_move={-1,-1,0};
        minimax_result to_choose_result=minimax(initial_dummy_move,3,true);
        //write the answer
        write_valid_spot(fout,to_choose_result.chosen_movement[0],to_choose_result.chosen_movement[1]);
    }
